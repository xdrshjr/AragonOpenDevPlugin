#!/usr/bin/env bash
# =============================================================================
# {{PROJECT_NAME}} — Dependency installer
#
...
if should_run python; then
  ...
  if [[ "$HAVE_PYTHON" == false ]]; then
    ...
    PYTHON_DIR="$PROJECT_DIR/{{BACKEND_SUBDIRECTORY}}"
    if [[ "{{BACKEND_SUBDIRECTORY}}" == "." ]]; then
    ...
    case "{{PYTHON_PACKAGE_MANAGER}}" in
      pipenv)
...
if should_run node; then
  ...
  if [[ "$HAVE_NODE" == false ]]; then
    ...
    NODE_DIR="$PROJECT_DIR/{{FRONTEND_SUBDIRECTORY}}"
...
echo -e "  Next steps:"
# {{IF HAS_FRONTEND}}
echo -e "    1. Build the project:  ${CYAN}{{BUILD_COMMAND}}${RESET}"
# {{ENDIF HAS_FRONTEND}}
